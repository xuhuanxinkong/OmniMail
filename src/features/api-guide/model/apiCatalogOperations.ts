...
export const iCloudEndpoints: ApiEndpoint[] = [
  ...
  {
    method: 'GET', path: '/api/icloud/tags', group: 'icloud', auth: 'authenticated',
    title: l('列出隐藏邮箱标签', 'List Hide My Email tags'),
    description: l('返回该 iCloud 账户下去重后的全部本地标签，用于标签筛选建议。', 'Return the account’s de-duplicated local tags for filter suggestions.'),
    request: 'Query · accountId', response: '200 · { tags }',
    examplePath: '/api/icloud/tags?accountId=icloud_account_id',
  },
  {
    method: 'GET', path: '/api/icloud/aliases/tags', group: 'icloud', auth: 'authenticated',
    title: l('读取隐藏邮箱标签映射', 'Read the alias tag map'),
    description: l('返回该账户下每个隐藏邮箱地址到其本地标签数组的映射。', 'Return a map of each alias email to its local tag names for the account.'),
    request: 'Query · accountId', response: '200 · { tags }',
    examplePath: '/api/icloud/aliases/tags?accountId=icloud_account_id',
  },
  {
    method: 'POST', path: '/api/icloud/aliases/tags', group: 'icloud', auth: 'authenticated',
    title: l('为隐藏邮箱添加标签', 'Add a tag to an alias'),
    description: l('给指定隐藏邮箱添加一个本地标签；重复添加已存在的标签不会报错。', 'Add a local tag to an alias; re-adding an existing tag is a no-op.'),
    request: 'JSON · accountId, aliasEmail, tagName', response: '201 · { ok: true, aliasEmail, tagName }',
    exampleBody: { accountId: 'icloud_account_id', aliasEmail: 'abc123@icloud.com', tagName: 'GitHub' },
  },
  {
    method: 'DELETE', path: '/api/icloud/aliases/tags', group: 'icloud', auth: 'authenticated',
    title: l('移除隐藏邮箱标签', 'Remove a tag from an alias'),
    description: l('从指定隐藏邮箱移除一个本地标签。', 'Remove a local tag from an alias.'),
    request: 'JSON · accountId, aliasEmail, tagName', response: '200 · { ok: true }',
    exampleBody: { accountId: 'icloud_account_id', aliasEmail: 'abc123@icloud.com', tagName: 'GitHub' },
  },
  {
